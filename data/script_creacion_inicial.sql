...
SELECT @cantidad = count(*) from DIVIDIDOS.pasajes where BOLETO_COMPRA_ID = @idBc
IF(@cantidad > 0)
begin
...
AS BEGIN
DECLARE @cantidad int
SELECT @cantidad = count(*) from DIVIDIDOS.paquetes where BOLETO_COMPRA_ID = @idBoletoCompra
IF(@cantidad > 0)
begin
	INSERT INTO DIVIDIDOS.cancelaciones (FECHA_DEVOLUCION, MOTIVO)
