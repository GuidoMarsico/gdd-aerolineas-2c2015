...
IF OBJECT_ID('AERO.roles_por_usuario') IS NOT NULL
BEGIN
    DROP TABLE AERO.roles_por_usuario;
END;
...
CREATE TABLE AERO.roles_por_usuario (
	ROL_ID    INT,
	USUARIO_ID INT, 
...
ALTER TABLE AERO.roles_por_usuario
ADD CONSTRAINT ROLES_POR_USUARIO_FK01 FOREIGN KEY
(ROL_ID) REFERENCES AERO.roles (ID)

IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'FKI_ROL_POR_USUA_ROL' AND object_id = OBJECT_ID('AERO.roles_por_usuario'))
    BEGIN
       CREATE INDEX FKI_ROL_POR_USUA_ROL ON AERO.roles_por_usuario (ROL_ID);
    END

ALTER TABLE AERO.roles_por_usuario
ADD CONSTRAINT ROLES_POR_USUARIO_FK02 FOREIGN KEY
(USUARIO_ID) REFERENCES AERO.usuarios (ID)

IF NOT EXISTS(SELECT * FROM sys.indexes WHERE name = 'FKI_ROL_POR_USUA_USUARIO' AND object_id = OBJECT_ID('AERO.roles_por_usuario'))
    BEGIN
...
UPDATE AERO.vuelos 
SET INVALIDO = 1
WHERE ID = @id AND FECHA_LLEGADA  IS NOT NULL  AND FECHA_SALIDA > CURRENT_TIMESTAMP
INSERT INTO AERO.cancelaciones (BOLETO_COMPRA_ID, FECHA_DEVOLUCION, MOTIVO)
